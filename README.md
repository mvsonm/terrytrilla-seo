...
| `lib/terrytrilla_seo/home_page.rb` + `plugin.rb` | `register_html_builder("server:before-head-close")` — meta tags of the custom home page | B13-bis: the theme draws its own home page, and core serves it as a bare shell with no meta tags at all (a crawler gets `custom_homepage_crawler_route` instead). Anything that identifies itself as a browser got the home page link with no description and no image. Tags are printed by core's own `crawlable_meta_data`, so the card is identical to every other page |
