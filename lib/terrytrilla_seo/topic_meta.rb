...
module ::TerrytrillaSeo
  ...
  module TopicMeta
    ...
    # ⚠️ Границы две, и верхняя куплена замером 18.09. У статьи «Как заменить аккорд
    # руками» первой картинкой идёт полоса ячеек 2124×246 — это 8.6:1. Прежнее правило
    # «шире полутора» считало её годной, и карточкой статьи стала тонкая лента, где
    # ничего не разобрать. Карточка ссылки — 1.91:1, и картинка должна быть похожей
    # с ОБЕИХ сторон: слишком узкую клиент покажет значком сбоку, слишком широкую
    # обрежет по краям или положит в поля. Не подходит ни то ни другое — рисуется
    # карточка сайта с заголовком на языке страницы.
    ШИРИНА_К_ВЫСОТЕ = 1.5
    ПРЕДЕЛ_ШИРИНЫ_К_ВЫСОТЕ = 2.4
    ...
    def self.широкая?(opts)
      ...
      return false if ширина < МИНИМАЛЬНАЯ_ШИРИНА
      отношение = ширина.to_f / высота
      отношение >= ШИРИНА_К_ВЫСОТЕ && отношение <= ПРЕДЕЛ_ШИРИНЫ_К_ВЫСОТЕ
    end
